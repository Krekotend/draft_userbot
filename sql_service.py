...
engine = create_async_engine('postgresql+asyncpg://username:password@localhost/database_name')
...
async def check_and_move_msg(session, source_table, destination_table, minutes: int):
    ...
    query = text(f"""
           ...
           JOIN status ON clients.status = status.status
           WHERE {source_table}.data <= :minutes_ago AND status.status = 'alive'
       """)

    results = await session.execute(query, {'minutes_ago': minutes_ago})
    msg_ids = [msg_id for msg_id, in results.fetchall()]  # Получаем все результаты и сохраняем только msg_id
    ...
    for msg_id in msg_ids:
        await session.execute(
            ...
            {'client_id': msg_id, 'current_time': current_time}  # Используем msg_id в качестве client_id
        )
        await session.execute(text(f"DELETE FROM {source_table} WHERE id = :msg_id"), {'msg_id': msg_id})

        client_ids_moved.append(msg_id)  # Добавляем msg_id в список
...
# проверяем есть ли client,если его нет записывем его в таблицу clients
async def chek_client_id(session, client_id):
    client = await session.execute(text("SELECT id FROM clients WHERE id = :client_id"), {'client_id': client_id})
    print('chek work and scherch')
    if not client[0]:
...
async def send_msg1():
    ...
    try:
        await check_and_move_msg(session, 'msg1', 'msg2', minutes=2)
    finally:
        await session.close()
...
async def send_msg2():
    session = await create_session()
    try:
        await check_and_move_msg(session, 'msg2', 'msg3', minutes=6)  # 39
    finally:
        await session.close()
...
async def send_msg3():
    ...
    try:
        current_time = datetime.now()
        minutes_ago = current_time - timedelta(minutes=10)
...
async def cancel_msg2(session, client_id):
    try:
        client = await session.execute(text("SELECT status_id FROM clients WHERE id = :client_id", {'client_id': client_id}))
